...
* 修复引魔香物品bug
  ...
  * 进入修炼清除引魔香状态
  * 断线重连不清除引魔香状态
    * 断线重连会进行一次重新进入房间的操作，若按原先的规则这里会出现引魔香状态会被清除，但期望是不要清除，故在进行重新进入房间操作之前对玩家角色set_temp一个临时标志，以此保证引魔香不被清除

* 增加背包扩容道具须弥石
  * 须弥石使用方式类似属性丹药，故将其按属性丹药操作，但策划要求须弥石以炼器的方式产出，最后的任务需求就变成了，通过炼器的方式产出一颗属性丹药
  * 须弥石使用后玩家角色身上"extra_capacity"字段增加，用于客户端包裹容量显示，同时玩家身上绑定的背包对象"extra_capacity"字段增加，这里才是真正的包裹容量增加

* web工具功能拓展及数据埋点工具相关
  * 优化一些报错提示
  * 修复自动查询、停止查询不生效的bug
  * taptap服过滤老玩家数据，qq服不过滤
  * 数据埋点洞府表,修复洞府日志格式，同时兼容老版本日志

* 修复道友录中存有没有名字的玩家客户端会报错的问题
  * 统一无名玩家名称为“神秘修士”

* 称号逻辑优化
  * 去抽不必要的set语句，同时称号进度更新仅更改变化的部分
  * 增加“狩猎者”称号
  * 优化称号界面的进度显示
  * 减少称号进度更新下发的数据

* 修复祭炼报错
  * 问题复现：祭炼法宝后去秘境挂机，挂机满8小时后登出，会触发该报错
  * 原因是玩家登出后切换到祭炼状态，会去尝试寻找一个不存在ob导致报错

* 数据埋点新增部分日志
  * 增加数据查询接口
    * 当前玩家身上所有装备中法宝的模板id（equipment表中的template字段），便于针对玩家选择倾向做数值调优
    * 当前玩家身上所有已获得神通的技能id和相应等级（skill表中的skill_id字段），便于针对玩家升级倾向做数值调优
  * 增加数据埋点日志
    * 记录玩家在各个秘境中的总停留时间（各秘境单独记录），便于调整秘境产出和难度数值
    * 记录玩家在各个境界等级的总停留时间（只记录突破之间的时间间隔，修法和炼体分开记录），便于调整修炼速度

## 第二十二周（10.8 - 10.12）

* 无名玩家相关
  * 修复道友录中存在没有名字的玩家的报错
  * 修复无名玩家正在重新起名的时候，其道友去读他的数据导致的报错
    * 从道友录中取好友数据分两种情况：在线、离线
    * 在线的时候去user表中取，而此时mongo中该玩家name字段为空，故会报错
    * 离线是去rank_list表中取，rank_list表保存的是之前该玩家有名字时候的字段，且在更新的时候若玩家没有名字则不更新到表中
  * 修复论剑台挑战无名玩家时切换界面导致的报错
    * 切换界面会恢复原来界面的信息，而论剑台对手的名字为空
  * 修复挑战无名玩家洞府导致的报错

* 修复炼器获取不到材料品质报错的问题
  * 客户端发上来一个错误的包，材料和配方不匹配，导致无法计算概率而报错

* web工具相关
  * web工具增加导出流量表功能
    * 之前没有完成的原因是，封装的XMLHttpRequest库不能读取excel文件，而报错的原因实际上是因为该库仅能读取文本文件，而excel显然不是
    * 解决方法即采用二进制的方式读取excel文件即可
  * web工具增加登陆gm账号功能
    * 之前是简单的把账号密码写在代码里的，太蠢了，故改良一下
  * web工具增加修改玩家账号密码的功能

* 版号申请相关
  * 更新敏感词库
    * 将策划给的word词库通过python脚本处理（他为毛要给我格式混乱的word）
  * 游戏中可输入文本的地方加入敏感词检测
    * 新增地方：问卷填空文本、宗门公告及简介
    * 客户端增加是否含有敏感词的功能接口(前缀树)
  * 进入游戏时，以广播形式播放健康游戏公告，仅自身可见
...
* 修复祭炼装备报错
  * 问题复现：祭炼法宝后去秘境挂机，后将该装备卖出/熔炼，挂机满8小时后登出，会触发该报错
  * 将祭炼状态下去秘境挂机，原先祭炼的装备限定不能出售
